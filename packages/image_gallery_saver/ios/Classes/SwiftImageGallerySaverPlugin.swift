...
public class SwiftImageGallerySaverPlugin: NSObject, FlutterPlugin {
    let errorMessage = "maybe permission error"

    var result: FlutterResult?;

    public static func register(with registrar: FlutterPluginRegistrar) {
      let channel = FlutterMethodChannel(name: "rhythm_native/image_gallery_saver", binaryMessenger: registrar.messenger())
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
      ...
      if call.method == "saveImageToGallery" {
        ...
        if status == .denied || status == .restricted {
            self.saveResult(isSuccess: false, error: "permission denied")
            return;
        }
